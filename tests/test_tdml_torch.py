...
def test_torch_eo_image_object_td():
    ...
    training_dataset = pytdml.io.read_from_json("tests/data/object-detection/COWC.json")  # read from TDML json file
...
def test_torch_eo_image_scene_td():
    training_dataset = pytdml.io.read_from_json("tests/data/scene-classification/WHU-RS19.json")  # read from TDML json file
    print("Load training dataset: " + training_dataset.name)
    print("Number of training samples: " + str(training_dataset.amount_of_training_data))
    print("Number of classes: " + str(training_dataset.number_of_classes))
...
def test_torch_eo_image_segmentation_td():
    ...
    training_dataset = pytdml.io.read_from_json(
        "tests/data/semantic_segmentation/GID-5C.json")  # read from TDML json file
    print("Load training dataset: " + training_dataset.name)
    print("Number of training samples: " + str(training_dataset.amount_of_training_data))
    print("Number of classes: " + str(training_dataset.number_of_classes))
